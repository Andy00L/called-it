...
/**
 * The score block (screen 01): teams, score or kickoff time, clock + phase
 * chip, then the probability pulse under a dashed rule. It renders bare so the
 * match cockpit can stack it with the pitch and the sponsor board in one card.
 */
  ...
  return (
    <div>
      <div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3.5">
        <h1 className="min-w-0 truncate text-right text-[22px] font-medium tracking-[-0.03em]">
          {participant1}
        </h1>
        <span className="tabular font-mono text-[28px] font-semibold">{centerValue}</span>
        <h1 className="min-w-0 truncate text-left text-[22px] font-medium tracking-[-0.03em]">
          {participant2}
        </h1>
      </div>
